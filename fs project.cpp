...
void login()
{
    ...
    // Mask the password with asterisks while reading
    while(1)
    {
        ch = getch();
        if(ch == 13) // Enter key pressed, exit the loop
            break;
        if(ch == 32 || ch == 9) // Ignore spaces and tabs
            continue;
        else
        {
            cout << "*";
    ...
    Password[i] = '\0'; // Add null character to the end of the password string
    ...
    else
    {
        ...
        system("cls");
        loginFrame1();
        login(); // Retry login after access denied
    }
}
...
void insert()
{
	...
	TAX =  0.04 * sal;
	DA = 1.20 * sal;
	PF = 0.12 * sal;//Provident Fund contributed by employee
	HRA = sal * 0.27; //House Rent Allowance
	MealA = 300;
	MedicalA = 300;
	TransportA = 300;
	LoanDeb = sal * 0.09;
	...
	emp[i].grosspay = (sal+MealA+MedicalA+TransportA+HRA+DA)-(PF+TAX+LoanDeb) ;
	  ofstream indexFile("index1.txt", ios::app);
    if (indexFile.is_open()) {
        indexFile << i <<setw(4)<<emp[i].code<<endl;
        indexFile.close();
    } else {
...
void intro()
{
    ...
    // Display "Designed and Programmed by" section
    gotoXY(20, 8);
...
void list()
{
    // Display a border without any delay
    borderNoDelay();
    
    // Display the title for the list of employees
    gotoXY(20, 4);
    ...
    // Iterate through the emp array to display the details of each employee
    for (int i = 0, j = 8; i <= num - 1; i++, j++)
    {
        // Display the details of the employee at specific positions
        gotoXY(6, j);
        cout << emp[i].name;
        gotoXY(19, j);
        cout << emp[i].code;
...
void menu()
{
	...
	printf("****** HR MANAGEMENT SYSTEM ****** ");
	gotoXY(12,6);
	cout<<"Press  i ----> Add New Employee.";
	gotoXY(12,8);
	cout<<"Press  e ----> Update an Employee.";
	gotoXY(12,10);
	cout<<"Press  d ----> Delete an Employee.";
	gotoXY(12,12);
	cout<<"Press  s ----> Search an Employee.";
	gotoXY(12,14);
 ...
 /*void deletes()  
{
    ...
    // Prompt the user to enter the job code to delete
    cout << "Enter the JobCode to Delete a Record: ";
    cin >> code;

    // Iterate through the emp array to find a matching record
    for (int i = 0; i < num; i++)
    {
        if (emp[i].code == code)
        {
            check = i;  // Record found, store its index
    ...
    if (check != -1)
    {
        // Shift subsequent records to fill the gap left by the deleted record
        for (int i = check; i < num - 1; i++)
        {
            emp[i] = emp[i + 1];
        }
        
        num--;  // Decrement the count of records
...
void deletes()
{
    ...
    if (check != -1)
    {
        ...
        // Update the "index1.txt" file after deleting the record
        ofstream indexFile("index1.txt");
        if (indexFile.is_open())
        {
            for (int i = 0; i < num; i++)
            {
...
void search()
{
    ...
    // Display information about the search criteria
    gotoXY(10, 4);
    ...
    for (int i = 0; i <= num - 1; i++)
    {
        ...
        {
            // Display header for the record details
            gotoXY(6, 8);
            cout << "Name\tCode\tDesignation\tYears(EXP)\tAge\tSalary " << endl;
            gotoXY(6, 9);
            cout << "------------------------------------------------------------------" << endl;
            
            // Display the details of the found record
            gotoXY(6, 11);
            cout << emp[i].name;
            gotoXY(19, 11);
            cout << emp[i].code;
...
void edit()
{
	...
	for(i=0;i<=num-1;i++)
	{
		if(emp[i].code==jobcode)
		{
			while((option=cin.get())!='q')  //checking wheather its not quit option
			{
				switch(option)
				{
					case 'n':
						editname(i);   //  goes to editname function
						break;
					case 'c':
						editcode(i);
						break;
...
void displayPayslip()
{
    // Clear the screen and set up the border
    system("cls");
    borderNoDelay();

    // Prompt for employee job code
    gotoXY(10, 4);
    int code, i;
    cout << "Enter Employee Job Code :";
    cin >> code;

    // Search for the employee with the given job code
    for (i = 0; i <= num - 1; i++)
    {
        if (emp[i].code == code)
        {
            // Display employee details and payslip
            gotoXY(12, 6);
